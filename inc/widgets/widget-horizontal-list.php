<?php
/**
 * Widget 6: Horizontal List Widget
 *
...
class Tahseen_Ashrafi_Horizontal_List_Widget extends WP_Widget {

    public function __construct() {
        parent::__construct(
            'tahseen_ashrafi_horizontal_list_widget',
            __('Tahseen Ashrafi - Horizontal List Widget', 'tahseen-ashrafi'),
            array(
                'description' => __('Widget 6: 2 rows of 3 posts (3 columns). Small square thumbnail left + title and meta right.', 'tahseen-ashrafi')
            )
        );
    }
    ...
    public function widget($args, $instance) {
        ...
        $post_count     = !empty($instance['post_count']) ? intval($instance['post_count']) : 6;
        ...
        if ($post_query->have_posts()) {
            echo '<div class="horizontal-list-grid-3col">';
            while ($post_query->have_posts()) {
                $post_query->the_post();
                $cat = get_the_category();
                $cat_name = !empty($cat) ? $cat[0]->name : '';
                ?>
                <div class="horiz-list-card">
                    <?php if (has_post_thumbnail()) : ?>
                        <div class="horiz-list-thumb">
                            <a href="<?php the_permalink(); ?>"><?php the_post_thumbnail('thumbnail'); ?></a>
                        </div>
                    <?php endif; ?>
                    <div class="horiz-list-body">
                        <?php if ($cat_name) : ?>
                            <span class="category-badge"><?php echo esc_html($cat_name); ?></span>
                        <?php endif; ?>
                        <h4 class="horiz-list-headline"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h4>
    ...
    public function form($instance) {
        ...
        $post_count     = isset($instance['post_count']) ? intval($instance['post_count']) : 6;
    ...
    public function update($new_instance, $old_instance) {
        ...
        $instance['post_count']    = !empty($new_instance['post_count']) ? intval($new_instance['post_count']) : 6;
