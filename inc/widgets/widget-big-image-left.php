<?php
/**
 * Widget 2: Big Image Left + News Grid
 *
...
class Tahseen_Ashrafi_Big_Image_Left_Widget extends WP_Widget {

    public function __construct() {
        parent::__construct(
            'tahseen_ashrafi_big_image_left_widget',
            __('Tahseen Ashrafi - Big Image Left News Grid', 'tahseen-ashrafi'),
            array(
                'description' => __('Widget 2: Left side (45%) large post with full-width image + Right side (55%) 2-column grid of smaller posts.', 'tahseen-ashrafi')
            )
        );
    }
    ...
    public function widget($args, $instance) {
        ...
        if ($post_query->have_posts()) {
            ...
            ?>
            <div class="big-image-left-container">
                <!-- Left Side (45%) -->
                <div class="big-image-left-col">
                    <?php if ($main_post) :
                    ...
                    ?>
                        <div class="big-left-card">
                            ...
                            <div class="big-left-body">
                                ...
                                <?php endif; ?>
                                <h3 class="big-left-headline">
                                    <a href="<?php echo esc_url(get_permalink($main_post->ID)); ?>"><?php echo esc_html(get_the_title($main_post->ID)); ?></a>
                                </h3>
                ...
                <!-- Right Side (55%) 2-Column Grid -->
                <div class="big-image-right-col">
                    <div class="small-news-grid-2col">
                        <?php foreach ($posts_array as $sub_post) :
                            $cat = get_the_category($sub_post->ID);
                            $cat_name = !empty($cat) ? $cat[0]->name : '';
                        ?>
                            <div class="small-grid-card">
                                <?php if (has_post_thumbnail($sub_post->ID)) : ?>
                                    <div class="small-grid-thumb">
                                        <a href="<?php echo esc_url(get_permalink($sub_post->ID)); ?>">
                                            <?php echo get_the_post_thumbnail($sub_post->ID, 'medium'); ?>
                                        </a>
                                    </div>
                                <?php endif; ?>
                                <div class="small-grid-body">
                                    <?php if ($cat_name) : ?>
                                        <span class="category-badge"><?php echo esc_html($cat_name); ?></span>
                                    <?php endif; ?>
                                    <h4 class="small-grid-headline">
                                        <a href="<?php echo esc_url(get_permalink($sub_post->ID)); ?>"><?php echo esc_html(get_the_title($sub_post->ID)); ?></a>
                                    </h4>
                                    <?php tahseen_ashrafi_render_post_meta($sub_post->ID, $show_author, $show_date, $show_views); ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
